        ...
        if (user.role_id) {
          RoleModel.findOne({ _id: user.role_id })
            .then(role => {
              ...
              res.send({ status: 0, data: user,role })
            })
        } else {
...
router.get('/auto_login', (req, res) => {
  ...
  if (!token) {
    return res.send({ status: 1, msg: '没有token，请先登录' })
...
router.post('/manage/category/delete', (req, res) => {
  ...
  // res.send({msg:'test',categoryId})
  CategoryModel.findOne({ _id: categoryId })
    .then(id => {
      if (id) {
        CategoryModel.deleteOne({ _id: categoryId }).then(resolve => {
          if (resolve) res.send({ status: 0, msg: '此分类已删除' })
        }, err => {
          console.log(err)
